...
### Q.7 — Cleanup Planning

Scope:
- aggressive audit of the consolidated `integrate/phase-Q` codebase to identify
  everything removable, consolidatable, or improvable before Q.8 execution
- two parallel workstreams:

  **arch-ctm — full integration branch audit:**
  - enumerate every lock site across all crates; classify removable vs must-retain
  - identify dead code, redundant abstractions, over-engineered seams
  - flag conservatively-held locks (wider or earlier than required)
  - identify consolidation opportunities across crate boundaries
  - produce a concrete removal/replacement inventory for Q.8

  **opus architect — full rust-best-practices review:**
  - run `rust-best-practices-agent` at Opus across the entire consolidated
    codebase (not round-limited)
  - identify systemic patterns to fix: error handling, type design, API surface,
    unsafe usage, performance anti-patterns
  - produce a prioritized findings list for Q.8 execution

- additional deferred items to plan:
  - FTQ-002: `acquire_env_lock()` fixed tmp path replacement
  - QA-003: in-process clear command test path for Windows CI
  - ATM-QA-008-001/002/004: auto-start test coverage gaps
  - any deferred minors from Q.4/Q.5/Q.6 QA rounds

Acceptance:
- lock audit complete with before-count, classification table, and removal plan
- rust-best-practices findings list complete and prioritized
- all deferred cleanup items have a concrete execution plan for Q.8
- no new implementation in this sprint — planning and audit only

Dependencies:
- Q.4 and Q.5 merged to `integrate/phase-Q`
- Q.6 release gate passed

### Q.8 — Cleanup Execution

Scope:
- execute all items planned in Q.7
- remove locks identified as removable in the Q.7 audit
- wrap remaining locks in RAII guards with guaranteed release
- fix FTQ-002: per-invocation unique lock path or nextest-safe serialization
- fix QA-003: in-process clear test path for Windows CI
- add auto-start test coverage (ATM-QA-008-001/002/004)
- close all deferred minors from Q.4/Q.5

Acceptance:
- no lock in any crate left unreleased on any exit path
- lock count reduced from Q.7 audit baseline
- Windows CI clear tests pass without daemon spawn
- auto-start success and failure paths covered by tests
- all deferred Q.4/Q.5 minors closed
