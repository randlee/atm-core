...
- retaining `atm peer resend-cache {show,set <true|false>}` only to report and
  persist disabled legacy state; `set true` is rejected, and the CLI never
  opens peer sockets, scans durable backlog, or retries delivery
