...
| `--handoff` |  | yes |  |
| `--reason` |  | yes |  |
| `--successor-task-id` |  | no |  |
| `--assign-to` |  | no |  |
| `--successor-priority` |  | no |  |
| `--assignment-text` |  | no |  |
| `<text>` |  | no | Plain text message body. Exactly one source is required |
| `--file` |  | no |  |
| `--stdin` |  | no |  |
| `--template` |  | no | Render a local template before requesting the durable task mutation |
| `--vars` |  | no | JSON object supplying variables to --template |
| `--team` |  | no |  |
| `--as` |  | no |  |
| `--stderr-logs` |  | no | Route retained observability console logs to stderr |
...
| Flag | Short | Required | Description |
|------|-------|----------|-------------|
| `<agent>` |  | no | Equivalent to --as for one selected assignee |
| `--as` |  | no |  |
| `--team` |  | no |  |
| `--closed` |  | no |  |
| `--limit` |  | no |  |
