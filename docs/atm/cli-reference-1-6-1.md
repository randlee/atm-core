...
| `--requires-ack` |  | no |  |
| `--task-id` |  | no |  |
| `--task-complete` |  | no | Close `--task-id` as completed after delivering this message as the report |
| `--dry-run` |  | no |  |
| `--json` |  | no |  |
| `--stderr-logs` |  | no | Route retained observability console logs to stderr |
...
| `<task_id>` |  | yes |  |
| `<outcome>` |  | yes |  |
| `<reason>` |  | no | Recorded on the close event; also the report body when no source is given |
| `<text>` |  | no |  |
| `--file` |  | no |  |
| `--stdin` |  | no |  |
| `--template` |  | no |  |
...
| `--alias` |  | no | durable roster alias; Herdr members use it as their live-agent target |
...
|------|-------|----------|-------------|
| `--team` |  | yes |  |
| `--kind` |  | yes | template kind: delivery, delivery_ack, queue, queue_ack, acknowledge, task_queued, task_ready, task_reminder, task_started, task_complete, task_closed; retired task and acknowledge_task are accepted for deletion |
| `--json` |  | no |  |
| `--stderr-logs` |  | no | Route retained observability console logs to stderr |
...
|------|-------|----------|-------------|
| `--team` |  | yes |  |
| `--kind` |  | yes | template kind: delivery, delivery_ack, queue, queue_ack, acknowledge, task_queued, task_ready, task_reminder, task_started, task_complete, task_closed |
| `--json` |  | no |  |
| `--stderr-logs` |  | no | Route retained observability console logs to stderr |
...
| `--session` |  | no | Herdr session name; only valid with --backend herdr |
| `--alias` |  | no | durable roster alias; Herdr members use it as their live-agent target |
| `--clear-alias` |  | no | remove the member's durable roster alias |
| `--pane-id` |  | no | deprecated compatibility spelling for --backend tmux --target |
| `--host` |  | no | this member's registered host (ADR-055 decision (e)); used for Send-To same-host/remote routing, never inferred |
| `--json` |  | no |  |
| `--stderr-logs` |  | no | Route retained observability console logs to stderr |
