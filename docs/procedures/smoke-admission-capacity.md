...
The `smoke-admission-capacity` procedure runs the named verification steps in order. Each step records an observable result in the run evidence. The page is addressed by the runner revision so a historical report remains explainable after the runner changes.
...
| --- | --- | --- | --- |
| 1 | Run `doctor` | PASS or FAIL is recorded | `smoke-admission-capacity.json` cases |
| 2 | Run `advertised host` | PASS or FAIL is recorded | `smoke-admission-capacity.json` cases |
| 3 | Run `smoke case results` | PASS or FAIL is recorded | `smoke-admission-capacity.json` cases |
