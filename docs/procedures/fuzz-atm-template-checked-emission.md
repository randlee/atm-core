...
evidence: site/reports/<run>/fuzz-atm-template-checked-emission.json
revisions:
...
This fuzz procedure runs the checked-emission campaign workers against bounded negative cases and retains each worker correlation id with its diagnostic contract result.

## Flow
```mermaid
flowchart LR
  setup[Setup fuzz-atm-template-checked-emission]
...
## Evidence layout
The runner writes immutable evidence for `fuzz-atm-template-checked-emission` beneath `site/reports/`. The JSON payload is the source for case or worker outcomes; the rendered report and envelope provide the public navigation entry.
