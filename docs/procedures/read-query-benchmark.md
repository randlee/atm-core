...
evidence: site/reports/<run>/read-query-benchmark.json
revisions:
...
The read-query benchmark compares mailbox query lanes against their reviewed baseline and preserves the campaign's lane-level verdicts and durability observations.

## Flow
```mermaid
flowchart LR
  setup[Setup read-query-benchmark]
...
## Evidence layout
The runner writes immutable evidence for `read-query-benchmark` beneath `site/reports/`. The JSON payload is the source for case or worker outcomes; the rendered report and envelope provide the public navigation entry.
