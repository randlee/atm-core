...
| ID | ADR-027 |
| Status | **Accepted** |
| Relates to | ADR-026, same-host RPC protocol |

## Decision

Before a client dispatches a write-shaped request over the admitted
`HostRuntimeScope`, it compares its normalized release version and wire
version with the connected daemon's compatibility verdict. A mismatch fails
closed with `ATM_CLIENT_DAEMON_VERSION_INCOMPATIBLE`; the client must install a
matching `atm`/`atm-daemon` pair and no write is dispatched.
