...
| HGC-012 / HGF-001 | P1 | Real `mode="steer"` falls back to queue | Production stores the direct agent in `SessionState.turn.agent`, but the seam unwraps only a tuple; tests manufacture the obsolete tuple shape | Accept the direct-agent representation, replace steer fixtures with production-shaped state, rerun the full suite, and delta-review a newly frozen SHA | blocking; assigned to Loki |
| HGC-013 / HGF-002 | P1 | A hung visible notice prevents the main internal event from routing | Soft-fail notice send has no deadline | Add a bounded deadline and prove timeout warns while the event routes exactly once | blocking; assigned to Loki |
| HGC-014 / HGF-003 | P1 | The documented frozen test procedure fails with `No module named pytest` | It installs with `--no-dev`, while pytest is in the `dev` extra | Correct the locked install/run commands and execute them exactly | assigned to Loki |
| HGC-015 / HGF-004 | P1 | Startup-hook test cannot detect loss of `gateway_runner` at the real emit site | It manually calls an `AsyncMock` with the expected payload | Exercise the production startup emit path or an extracted production payload builder | assigned to Loki |
