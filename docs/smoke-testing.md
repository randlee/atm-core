...
just smoke fast
just smoke thorough              # includes the same-host graft lane
...
| graft-hermes | `scripts/phase-ai/run-hermes-graft-smoke.py` | Full PyO3/Hermes graft surface |
...
| Standard smoke implementation | `scripts/smoke/run_feature_smoke.py`, `run.py`, `run_thorough.py`, `run_thorough_shared_host.py`, `run_graft_same_host.py`, `run_peer_pair.py`, `run_inbound_peer_smoke.py`, `combine_inbound_peer_smoke.py` | Internal to `just smoke`; never invoke directly. |
...
| Specialized graft implementation | `scripts/phase-ai/run-hermes-graft-smoke.py` and its unit-test helpers | Internal to `just smoke graft-hermes`. |
