...
## 0. Mechanical enforcement inputs

Treat these as mandatory evidence when reviewing boundaries in `atm-core`.
They are not optional background reading:

- `boundaries/**/*.toml` — authoritative boundary records and allowed edges
- `.just/lint_boundaries.py` — primary boundary-policy enforcement script
- `.just/lint_manifests.py` — manifest-layer dependency enforcement
- `.just/run_lint.py` — lint entrypoint used to invoke boundary/manifests checks
- `crates/atm-architecture/tests/boundary_enforcement.rs` — executable proof
  that the boundary records still match repo structure
- `crates/sc-lint-boundary/config/defaults.toml` — default severity and policy
  behavior for boundary linting
- `docs/sc-lint/README.md` — operator-facing overview of the sc-lint boundary
  model
- `docs/sc-lint/boundary-enforcement-model.md` — enforcement model and
  lifecycle expectations
- `.claude/agents/boundary-guard.md` — reviewer prompt for strict
  boundary-policy verification

If a repeated leak pattern is real in code but none of these surfaces can
catch it mechanically, that is a `lint_gap` finding.
