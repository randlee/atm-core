...
| Z.2 | `PENDING` | `FAIL` | `complete` | frozen `Z.1` smoke checklist rerun completed on `integrate/phase-Z`; `Z1-F001` and `Z1-F002` both reproduced, and new blocking regression `Z2-F001` reopened `Z1-003`; canary-entry decision is `stop` and `Z.3` may not begin |
