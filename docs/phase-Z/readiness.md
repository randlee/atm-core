...
| Z.2 | `6640a61b` | `FAIL` | `complete` | frozen `Z.1` smoke checklist rerun completed on `integrate/phase-Z`; `Z1-F001` and `Z1-F002` both reproduced, and new blocking regression `Z2-F001` reopened `Z1-003`; canary-entry decision is `stop` and `Z.3` may not begin |
...
| Z.3 | `PENDING` | `PENDING` | `not started` | awaits `Z.2` blocker closure plus the accepted `Z.11` through `Z.14` follow-up line before canary entry |
| Z.4 | `PENDING` | `PENDING` | `not started` | awaits `Z.3` closure |

## Deferred Follow-Up Findings

- `RBP-F002` deferred to a later follow-up line: introduce a dedicated `PaneId` newtype for
  Claude pane metadata instead of the current `Option<String>` surface.
- `RSH-008` deferred to a later follow-up line: add the payload-size gate in
  `deliver_payloads` rather than widening `Z.6` scope.
- `RSH-010` deferred to a later follow-up line: close the remaining send-path hardening item
  tracked by QA after the `Z.6` runtime-roster-view cutover.
- `RBP-F003` deferred to a later follow-up line: tighten the remaining Rust best-practices
  follow-up on the `Z.6` roster-view surface.
- `RBP-F004` deferred to a later follow-up line: tighten the remaining Rust best-practices
  follow-up on the `Z.6` roster-view surface.
- `RBP-F005` deferred to a later follow-up line: tighten the remaining Rust best-practices
  follow-up on the `Z.6` roster-view surface.
- `RSH-009` deferred to a later follow-up line: close the remaining runtime hardening follow-up
  after `Z.6`.
- `ATM-QA-Z10-001` deferred to a later follow-up line: complete the remaining `Z.8`
...
- `RSH-001` daemon join-handle tracking hardening deferred to a later follow-up line
- `RSH-002` daemon runtime shutdown-path hardening deferred to a later follow-up line
- `RSH-003` daemon lifecycle edge-case hardening deferred to a later follow-up line
- `RSH-004` daemon control-path hardening deferred to a later follow-up line
- `RSH-005` daemon signal-path hardening deferred to a later follow-up line
- `RSH-006` daemon worker coordination hardening deferred to a later follow-up line
- `RSH-007` daemon platform/runtime hardening deferred to a later follow-up line
- `RSH-009` local IPC timeout-path tracing hardening deferred to a later follow-up line
- `RBP-F001` typed newtypes for `RosterMemberRecord.agent_type` / `.model` deferred to a later follow-up line
- `RBP-F003` typed `ResolvedTarget` hardening deferred to a later follow-up line
- `RBP-F004` structured error-diagnostics hardening deferred to a later follow-up line
- `RBP-F005` replayed local IPC path hardening deferred to a later follow-up line
- `M-004` stronger typed/length-constrained `AddMemberRequest.agent_type` / `.model` deferred to a later follow-up line
