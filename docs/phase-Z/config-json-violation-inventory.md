...
| `crates/atm-core/src/boundary_support.rs` | helper loaded team config through a generic boundary support seam | narrow | `Z.7` | `closed` | only workspace-config loading plus the named startup-only helper remain; generic runtime lookup support was deleted |
| `crates/atm-core/src/direct_boundaries.rs` | forwarded generic `load_team_config(...)` boundary calls | narrow | `Z.7` | `closed` | generic runtime lookup forwarding was deleted; the only surviving team-config forwarder is the named startup-only helper that sunsets in `Z.8` |
| `crates/atm-core/src/boundary_support.rs` `hydrate_roster_from_team_config_once_at_startup_if_empty(...)` plus `crates/atm-core/src/direct_boundaries.rs` forwarding | temporary startup-only one-shot roster hydration when canonical ATM roster is empty before `Z.8` watcher ingress exists | keep, narrow then delete | `Z.7` | `closed` | helper renamed, allowlisted in `.just/allowlists/scb_config_allowlist.toml` with `sunset_sprint = "Z.8"`, and covered by a non-empty-roster no-op test; `Z.8` deletes the helper and forwarding once watcher / reconcile becomes the sole external ingest path before `Z.3` begins |
