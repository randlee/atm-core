...
| `crates/atm-core/src/boundary_support.rs` `hydrate_roster_from_team_config_if_empty(...)` plus `crates/atm-core/src/direct_boundaries.rs` forwarding | temporary startup-only one-shot roster hydration when canonical ATM roster is empty before `Z.8` watcher ingress exists | keep, narrow then delete | `Z.7` | `open` | `Z.7` must rename and allowlist this exact bootstrap-only helper, prove it is a no-op when ATM roster is non-empty, and forbid any generic caller; `Z.8` deletes the helper and forwarding once watcher / reconcile becomes the sole external ingest path before `Z.3` begins |
