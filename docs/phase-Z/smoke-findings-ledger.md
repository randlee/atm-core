...
| `Z1-F002` | `Z.1` | `Z1-008` | Current-state `mail.db` cannot pass current SQLite schema initialization, so daemon auto-start never publishes IPC and daemon-backed retained commands fail on the copied real-state baseline. | `blocking` | `arch-ctm` | `closed` | `closed in Z.16 via schema compatibility preflight` | `PASS` | Closed on `f412959d`. `ensure_schema(...)` now adds the compatibility `message_id` column on legacy `mail_messages` tables before the migration batch creates `uq_mail_messages_message_id`, and the new regression test `ensure_schema_upgrades_legacy_mail_message_identity_shape` proves the older `legacy_message_id` schema upgrades successfully. Revalidation on a disposable copy of `~/.claude/teams/atm-dev` plus `~/.atm/db/mail.db` showed `doctor`, `list`, `send`, and `read` no longer fail with `failed to initialize sqlite schema`. |
