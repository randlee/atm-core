...
- canonical shared `Message`, `Roster*`, and `Task*` structs are used at both the RPC body layer and the storage layer
- Claude storage and the SQLite backend implement the same core storage traits
