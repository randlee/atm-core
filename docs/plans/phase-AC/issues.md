...
| `AC-ISSUE-002` | `closed` | message and roster records are duplicated across RPC, storage, and internal boundaries instead of using canonical shared structs; speculative task-store records also exist and must not be promoted into the approved shared contract. | `AC.1`, `AC.5`, and `AC.6` |
...
| `AC-ISSUE-004` | `closed` | the concrete SQLite backend has become the implicit home of business logic, causing backend-specific seams and logic leakage upward. | `AC.3`, `AC.4`, and `AC.6` |
