...
| --- | --- | --- | --- | --- |
| AF1-D1 | ADR update and shared `HostRuntimeScope` boundary | `docs/adr/ADR-002-host-wide-daemon-singleton.md`, `crates/atm-core/src/home.rs` (or dedicated runtime module) | ADR states the OS-user scope, state-root mismatch behavior, exact three barriers, and that no environment/test override exists. All runtime path derivation is centralized behind `current_host_runtime_scope`. | Unit tests cover path construction without consulting forbidden environment variables; source inventory identifies every former `host_runtime_lock_path*` user. |
...
- Select platform implementations for a persistent, OS-account-derived runtime
  directory (macOS/Linux and Windows) and document permission/symlink checks.
- Decide and document the user-facing state-root mismatch error code and the
  migration from the current `ATM_HOME/.atm/daemon` lock/socket layout.
- Map every `ATM_HOME`, `ATM_DAEMON_SOCKET`, and direct-daemon call site; the
  review is blocked until each is classified as state configuration, rejected
  override, or removed.
