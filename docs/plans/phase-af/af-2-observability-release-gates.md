...
newtype; the compatibility preflight runs after local-IPC connection but before
any write-shaped request dispatch. An incompatible verdict is returned with
`ATM_CLIENT_DAEMON_VERSION_INCOMPATIBLE`, recovery guidance, and no write.
