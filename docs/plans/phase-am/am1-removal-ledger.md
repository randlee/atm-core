...
| AM1-RM-002 | `atm-daemon-client` synchronous local client: `http_exchange`, `try_connect`, `exchange_request`, compatibility/bootstrap support.  `cargo tree -i` and caller search confirm `atm` and `atm-graft` still use it only for daemon availability plus non-write read/ack/admin compatibility dispatch. | **Retained after AM.3 review.** AL's async runtime client owns canonical writes; the synchronous compatibility wrapper remains until a later owner migrates the non-write contract.  Its Cargo edges and callers are therefore not AM.3 deletion targets. | `cargo tree -i atm-daemon-client`; caller search in `crates/atm` and `crates/atm-graft`; no write path may select this wrapper.  RM-002 remains a predecessor of RM-001. |
