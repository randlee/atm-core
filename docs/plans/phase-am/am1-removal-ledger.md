...
| --- | --- | --- | --- |
| AM1-RM-001 | `atm-core/src/api/http_frame_reader.rs` and raw helpers exported by `api.rs`: `HttpFrameReader`, request/response readers/writers and `decode_request`.  After AM.3, live callers are the retained `atm-daemon-client` non-write compatibility exchange, `atm-http-runtime`'s request conversion, and direct tests. | **AM.2 owns the complete caller-before-callee transition:** migrate the retained compatibility exchange to the shared typed client, move runtime request conversion to the typed runtime boundary, then remove the callee and its direct tests/exports.  `atm-http-runtime::{client,http1_server,message_handler}` is the replacement boundary. | Search first command before and after each migration; the final search must be empty.  Local and cross-host smoke follow the deletion.  Follows RM-002 and RM-003. |
