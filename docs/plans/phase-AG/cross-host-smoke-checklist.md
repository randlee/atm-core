...
| `AG-VAL-011` | `Lane A` | `Windows/macOS` | `Windows/macOS` | transport-security requirement disposition | inspect retained evidence for cross-host transport against documented TCP/TLS requirement and record whether TLS is actually present | AG either captures affirmative TLS evidence or opens/links a named `PRODUCT-BUG` / requirement-drift finding that blocks any release-usable verdict | requirement/architecture citation; implementation citation; finding link if TLS is absent; final verdict linkage | `PENDING` | `—` | mandatory before AG.5 may issue any release-usability claim |
