---
status: complete
branch: docs/ba6-task-nudge-documentation
worktree: /Users/randlee/Documents/github/atm-core-worktrees/docs/ba6-task-nudge-documentation
---
...
- [x] D1 — `CLAUDE.md:227` (resend assignment via `atm send`) and the ATM
  ...
  the current task".
- [x] D2 — `docs/team-protocol.md:11`: closing = `atm task close <task-id>
  ...
  assign.
- [x] D3 — `docs/requirements.md:2482` doctor remediation literal → the
  string the shipped code emits.
- [x] D4 — `docs/atm-daemon/http-api.md`: the final `1.6.0` surface —
...
- [x] D5 — ADR-062 Phase-BA amendment: replace "R1 pending" wording with the
...
- [x] D6 — `docs/project-plan.md`: Phase BA section (six sprints, status),
  AZ section already marked retired.
- [x] D7 — `docs/plans/phase-ba/phase-ba-plan.md` §2: each R-row gets its
...
- [x] D8 — `docs/architecture.md` Task Storage: the plan §1 task-state table,
...
- `just lint adr-index` and `just lint manifests` (documentation index/link
  and manifest validation) pass.
- `cargo test -p agent-team-mail --features cli-surface-dump --test
  cli_surface` passes against the live clap command tree.
- `grep -rn "atm list --tasks\|--task-complete <\|task close .* --outcome"
  CLAUDE.md docs/*.md` returns no matches.

## Acceptance criteria

1. `grep -rn "atm list --tasks\|--task-complete <\|task close .* --outcome" CLAUDE.md docs/*.md` → nothing.
2. `cargo test -p agent-team-mail --features cli-surface-dump --test
   cli_surface` passes.
...
`just lint adr-index`; `just lint manifests`; `cargo test -p agent-team-mail
--features cli-surface-dump --test cli_surface`; and the AC1 grep above. The
phase-ending critical review (five reviewers on the integrate head) runs after
this sprint merges.
