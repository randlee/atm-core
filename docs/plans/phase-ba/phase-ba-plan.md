...
| BA.2 | `TaskStateTag` (scalar wire tag); `TaskRowWire`, `TaskEventRowWire` (serde `try_from`/`into` shapes; `TaskState::from_parts`, `tag()`, `close_outcome()`); `TaskOp { Start, Close }` (no message-carried move — design §5), `MoveTarget { Head, End, Before }` (used only by BA.4's `TaskMoveRequest`); `WriteRequest.task_op` (additive; `task_complete` kept decode-only) and `WriteRequest::task_op_normalized`; `TaskCloseApplied { task_id, assignee, outcome, consecutive_refusals }` carried as `SendOutcome.task_close`; `TASK_CONSECUTIVE_REFUSAL_THRESHOLD`; `HTTP_API_VERSION` 1.5.0 |
...
| BA.3 | `herdr_task_disposition.rs`: `TaskDisposition`, `EpisodeKind` (+ `as_str`), `HoldReason` (incl. `NoDeliveryChannel`), `dispose`, `reminder_due`; `TASK_REMINDER_INTERVAL_MS` moved to `atm-storage/src/task_store.rs` (`i64`); `EscalationState` reshaped to `{episodes}` with `Episode { kind, since, notified }`, `observe`, `mark_notified`; `episode_summary`, `episode_already_reported(target, summary, since)`; `escalate_mail(…, suppress_since)`; start-owed retry of `TaskOp::Start` for reminded `assigned` heads; `EscalationKind::RefusalsEscalated` (`BreakerOpened` deleted); `MemberObservation`; `task_started` template text (existing template class, no new nudge kind) |
