...
| `complete` | → `assigned` (reopen in place; event `reopened`) | ordinary mail write returns `already_closed` | ordinary mail write returns `already_closed` — **informational** at the command layer (B10) |
...
sender (unchanged); `Reassigned` / `Reopened` — the assignment operation has
no caller check; `Started` — the daemon (**§4 R1**); `Completed` — the
...
| **R8** | **Host-qualified task targets.** `AgentAddress` accepts `agent@team.host`, but a peer write never transitions the ledger (ADR-062) and `TaskRow` carries no host, so a cross-host task would have no ledger row on either host and no return route for `task_started` or the close report. | **Task commands are local-team only in Phase BA.** `atm task assign`, `atm task close`, and `atm send --task-id` reject an assignee/recipient whose resolved team differs from the caller's team or whose host is set, at CLI preflight before any send: `AtmError::validation("task commands are local-team only; <addr> resolves to another team or host — send a plain message or assign the local alias")`, exit 1, zero mail, zero rows. The writer refuses `task_op` on a `to` whose team differs from the caller's team or whose host is set with the same validation error (defense in depth; nothing reaches the peer wire). | A canonical owning host with cross-host receipts/reports (new authority model; design-excluded, deferred to a later phase). |
...
| BA.2 | `QueuePosition(NonZeroU32)` newtype; `TaskCloseOutcome`; `TaskState::Complete(TaskCloseOutcome)`; transition input `TaskEvent::{Assigned, Started, Completed(outcome)}`; audit `TaskEventKind::{Assigned, Started, Reassigned, Reopened, Completed, Refused, Cancelled, Moved, Migrated}`; `TaskRejected { kind }` + five `TaskRejectionKind` variants (NoOpenTask, NotAuthorized, ActiveElsewhere, UnknownTarget, StaleCounterparty); `TaskRow.position`; `Transition(pub TaskState)` struct |
| BA.2 | `TaskStateTag`, `TaskRowWire`, `TaskEventRowWire`, `TaskOp { Start, Close }`, `MoveTarget { Head, End, Before }`; `WriteRequest.task_op` plus optional `placement` copied unchanged through the persisted envelope (None = END); `RefusalRun { count: u32, started_at: Option<IsoTimestamp> }` and `AsyncTaskLedgerReader::refusal_run(team, assignee)`; `HTTP_API_VERSION` 1.5.0 |
...
| --- | --- | --- |
| task op on a missing / foreign task; second active task; `--before` naming another member's task | `TaskRejected { kind: TaskRejectionKind::{NoOpenTask, ActiveElsewhere, UnknownTarget} }`; a complete task is reopened by explicit `assign`; over HTTP only as `AtmError` code + message | BA.2 writer |
| close message recipient is no longer the current counterparty (task reassigned between preflight and write) | `StaleCounterparty`: atomic rejection, nothing written; CLI prints the writer message, exits 1, and tells the user to re-run |
| close repeated after completion | ordinary mail write returns `already_closed`: report delivered, `task <id> was already closed (<outcome>); report delivered`, exit 0, no task event |
...
The writer atomically rejects a stale counterparty; the CLI prints the writer
message, exits 1, and sends nothing.
