...
| **R8** | **Host-qualified task targets.** `AgentAddress` accepts `agent@team.host`, but a peer write never transitions the ledger (ADR-062) and `TaskRow` carries no host, so a cross-host task would have no ledger row on either host and no return route for `task_started` or the close report. | **Task commands are local-team only in Phase BA.** `atm task assign`, `atm task close`, and `atm send --task-id` reject a host-qualified assignee/recipient at CLI preflight before any send: `AtmError::validation("task commands are local-team only; <addr> is on another host — send a plain message or assign the local alias")`, exit 1, zero mail, zero rows. The writer refuses `task_op` on a host-qualified `to` with the same validation error (defense in depth; nothing reaches the peer wire). | A canonical owning host with cross-host receipts/reports (new authority model; design-excluded, deferred to a later phase). |
