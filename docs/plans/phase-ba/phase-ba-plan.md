...
| BA.5 | `OPEN_ITEM_SQL`; `PendingNudgeStore::rearm_pending_after_handoff` (rename of `clear_pending_on_handoff`); `clear_pending_on_read` deleted; `mark_message_read` close-or-rearm `CASE`; `requeue_pending` interval back-off at `MAX_NUDGE_ATTEMPTS`; `nudge_dispatch::rearm_queue_marker_after_handoff` (rename); `next_reminder_due`; claim predicate `nudge_pending_at <= now` |
| BA.6 | nothing |
