...
| `parallel_safe` | BA.4 — this sprint owns `crates/atm-storage-rusqlite/src/pending_nudge_store.rs`, the `PendingNudgeStore` trait block in `crates/atm-storage/src/contract.rs:1290-1320`, `crates/atm-core/src/nudge_dispatch.rs`, `herdr_queue_wake.rs::complete_successful_claim`, and every rename site of the handoff helper: `storage_and_nudge_router.rs:1240-1245` (adapter only), `crates/atm-daemon-bootstrap/src/{lib.rs:1385, queue_drain.rs:417, received_hook_selector.rs:617}`, `crates/atm-core/tests/nudge_mode.rs:132`, `crates/atm-architecture/tests/boundary_enforcement.rs:566-623`. BA.4 touches none of these. |
...
| --- | --- | --- |
| D1 | `OPEN_ITEM_SQL` predicate; `claim_next_pending` eligibility adds `nudge_pending_at <= now` and uses the predicate (so an acked item is closed without touching the ack writer); `rearm_pending_after_handoff` (replaces `clear_pending_on_handoff`); `clear_pending_on_read` closes-or-rearms | `crates/atm-storage-rusqlite/src/pending_nudge_store.rs` |
| D2 | `PendingNudgeStore` trait: `clear_pending_on_handoff` → `rearm_pending_after_handoff(member, msg, next_due)`; doc comments state the due-at semantics | `crates/atm-storage/src/contract.rs:1290-1320` |
...
| `requeue_pending` (`:94-97`, failed dispatch) | `SET nudge_pending_at = now, nudge_attempts = attempt + 1` | unchanged — immediate retry, bounded by `MAX_NUDGE_ATTEMPTS = 5` (`contract.rs:1238`) |
...
| `clear_pending_on_read` (`:135-140`) | `SET nudge_pending_at = NULL` | `SET nudge_pending_at = CASE WHEN pending_ack_at IS NOT NULL AND acknowledged_at IS NULL THEN ?next_due ELSE NULL END` — closed-on-read, or re-armed until the ack (design §9 close trigger) |
...
rate limit for messages and tasks). `nudge_attempts` keeps its meaning
(failed dispatches only) and its cap; a successfully prompted item that is
simply not read is re-armed indefinitely at the interval, exactly like a
task reminder, until it closes. There is **no** terminal threshold for a
...
// crates/atm-storage/src/contract.rs — PendingNudgeStore (sealed), changed method
    ...
    /// Closes the item on read, or re-arms it when an ack is still owed.
    fn clear_pending_on_read(&self, member: &MemberKey, msg: &AtmMessageId, next_due: IsoTimestamp) -> Result<(), AtmError>;
```
...
- `read_closes_item_without_ack_requirement`.
- `read_rearms_item_when_ack_owed` — `pending_ack_at` set, not acked →
  `nudge_pending_at == next_due`; after `acknowledged_at` is set the item is
  never claimed again (marker value irrelevant).
...
- `read_message_is_never_reminded` — `atm read` before the first interval
  → 0 further prompts.
- `requires_ack_message_reminded_until_acked` — read but unacked →
  reminded at interval; `atm ack` → silence.
...
- `exhausted_deferred_dispatch_stops_at_max_attempts` — force 5 failed
  dispatches → no further claims; unchanged behaviour, pinned.
...
3. `grep -rn "clear_pending_on_handoff\|clear_queue_marker_after_handoff" crates/` → nothing.
