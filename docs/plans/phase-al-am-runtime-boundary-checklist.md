...
| One async client boundary | The existing sealed `DaemonApiClient` is the only asynchronous client dispatch boundary and uses `#[async_trait]` to retain object-safe `Arc<dyn ...>` use. No second client trait, manual future vtable, or blocking bridge exists; `MessageReceivedHookEmitter` remains synchronous. |
