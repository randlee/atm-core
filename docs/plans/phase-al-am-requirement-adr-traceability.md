...
| `REQ-CORE-TRANSPORT-005`; `REQ-P-DAEMON-DISPATCHER-001`; `REQ-DAEMON-TRANSPORT-003`, `004` | Reuse the sealed `DaemonApiClient`; one absolute request budget; framework request work is drain-tracked and bounded. | AL.1, AL.4–AL.9 | compile/boundary test, Tokio-time deadline test, shutdown drain/cancel test |
| `REQ-CORE-TRANSPORT-006`; `REQ-DAEMON-TRANSPORT-006`, `007` | The requirement's custom-framing implementation is historical, not a retained compatibility surface. AL prevents replacement raw framing and AM.2 deletes the legacy implementation; maintained Tokio HTTP owns protocol mechanics. UDP remains prohibited. | AL.1 prevents new code; AM.2 removes legacy code | dependency/symbol negative tests and smoke after deletion |
...
| `REQ-DAEMON-TRANSPORT-008` | Preserve the existing UDS/loopback endpoint-selection contract while moving only physical connection setup into adapters; no silent fallback or alternate application route. | AL.5, AL.6 | endpoint-selection regression tests, UDS owner/capability negatives, Windows loopback result |
...
| `REQ-CORE-TRANSPORT-005` capacity/body/deadline clauses; ADR-033 | Apply existing documented body, connection, and shutdown limits through framework configuration/middleware. Do not create ATM socket loops or a new scheduler. | AL.4–AL.7, AL.9 | configuration test, body-limit test, deadline test, and shutdown test; **AL.9 only** owns the benchmark artifact and closure gate |
| Archived AK.11 `88bca9d5` `MessageReceivedHookEmitter` semantics | Receive hook is injected, receiver-only, post-new-persistence, duplicate-suppressed, warning-only on failure. `PostSendHookEmitter` has no active runtime reference. | AL.1 narrow exact-copy gate, AL.3 | three deterministic hook tests and dependency/call-site guard |
