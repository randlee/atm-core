...
| PF-2 crates.io state/liveness check | The raw `/api/v1/me` curl returns 403 while the same configured credential successfully published crates using Cargo. | The `crates-io` channel agent receives the manifest-derived crate list and emits a fenced JSON receipt for **every** crate: crate name, current published version(s), newest/current version, last-publish timestamp, explicit classification — `new` (no crate exists), `existing` (crate exists but requested version is absent), or `version_conflict` (requested version already exists) — and the manifest-derived `planned_action`: `publish`, `skip_already_at_target`, or `block_conflict`. Preflight consumes that receipt for registry state instead of treating a raw `/api/v1/me` probe as equivalent to Cargo publish authentication. The receipt contains no credential value. Do not rotate or replace a working production token based on the raw curl. |
