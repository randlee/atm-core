...
| PEP 621 dynamic Python version | The canonical manifest validator rejects ATM's valid Maturin `dynamic = ["version"]` metadata, derived from Cargo. | Upstream `sc-publish` must support the declared Cargo-derived version. ATM must convert every published Python distribution to that same dynamic Cargo-derived form and enforce resolved-version lockstep; do not retain a literal Python version merely to satisfy a premature shared parser. |
