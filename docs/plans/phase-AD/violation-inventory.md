...
| `crates/atm-core/src/read/mod.rs`, `crates/atm-core/src/read/metadata_selection.rs`, and read-mutation smoke/test coverage | Historical read paths could report the original `selected_message_id` while returning the next unread payload. The accepted reader-lane contract returns the selected message and a snapshot that MAY predate durable handoff application. | Preserve the selected message as the returned payload (or return no payload if that is the deliberate contract), report handoff acceptance separately, and require a bounded later list poll when durable visibility is needed. |
