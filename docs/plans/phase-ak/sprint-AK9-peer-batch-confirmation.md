...
next attempt; it does not infer which remote messages might have arrived.
...
2. Replace singular peer-delivery confirmation with
   ...
   and due-page callers together; do not leave a compatibility sender path.
...
5. Update ADR-046/ADR-047, requirements, daemon/storage boundaries, and
   ...
   atomic outbound confirmation. AK.6 may then update only supersession/status
   cross-references.
...
merge. AK.6 may continue its isolated fixture work, but must merge AK.8 and
AK.9 before final validation, every final fix round, and PR completion.
