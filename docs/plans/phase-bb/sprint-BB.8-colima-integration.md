...
- [ ] D3 — `templates/integration-report/run.html.j2`, `templates/integration-report/step-panel.xhtml.j2`.
...
- [ ] D5 — `.just/generate_report_index.py` integration family + history page; `just reports-index` output committed.
- [ ] D6 — twelve historical runs under `site/reports/integration/colima/`, payloads byte-identical (prove with `git diff -M100% --name-status`).
- [ ] D7 — evidence guard accepts `R100` renames; test for it.
