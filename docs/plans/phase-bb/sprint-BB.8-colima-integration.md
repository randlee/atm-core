...
- The driver (`scripts/integration/run_colima.py`, `just integration colima`)
  runs the testbed's canonical `test.sh` exactly once, converts its seven
  reports with `colima_skill_report.py`, then invokes Tier E prompts AT9,
  AT10, and AT11 in the live fixture. Real agents make all product
  observations through public ATM CLI JSON and write `prompt-report-1`.
  The driver only collects those reports and renders them. A failing step is
  recorded and later prompts still run; aggregate PASS requires all four.
- The host-side `run_bb{4,5,6}_*.py` simulators, their tests, and the driver
  reset/schema inventory are deleted. The historical payloads they produced
  remain byte-identical and continue rendering through
  `scripts/integration/render_colima.py`.
...
| BB.8.2 | `feature/bb8-2-colima-driver`, cut from the top of BB.8.1 | thin live driver (D1), testbed Tier E prompts AT9–AT11, deletion of host simulators/reset internals (D2), sequence procedures (D4 rest), one real run (D8), driver tests (D9 rest), frontmatter `status: complete` (D10 rest) | yes |
...
- [x] D1 — `scripts/integration/run_colima.py` + `just integration colima [--out DIR]`: one `test.sh`, then AT9–AT11 in its live fixture.
- [x] D2 — host simulators, database reset policy, and reset schema test deleted; `colima_skill_report.py` remains the canonical `test.sh` converter; no dead code.
- [x] D3 — `templates/integration-report/run.html.j2`, `templates/integration-report/step-panel.xhtml.j2`.
- [x] D4 — `docs/procedures/colima-integration.md`, `colima-task-start.md`, `colima-assignment.md`, `colima-prompt-handoffs.md`; `colima-hermes-skills.md` family → integration; historical revisions retained; pages regenerated by `just procedures`.
