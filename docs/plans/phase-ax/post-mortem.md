...
| `AXPE-QA-115` — `tools/bootstrap.toml` pins rust 1.94.1 vs. cargo-shear 1.13.2 compatibility, blocking local `just validate`/`bootstrap` | important | `planning_process_improvement` (tooling debt, tracked separately per `project_bootstrap_toolchain_version_debt.md`) | `tools/bootstrap.toml` |
