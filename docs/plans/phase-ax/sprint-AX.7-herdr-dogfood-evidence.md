...
| C18 | assign a task to cipher, then have cipher hit an approval prompt (e.g. a tool call needing confirmation) and leave it ≥ 2 min | `atm members` shows cipher `state=blocked`; doctor warns `ATM_MEMBER_BLOCKED`; within ~65 s team-lead receives the C3 blocked message from `atm-daemon`, a Herdr notification appears on screen, and (if a daemon-scope escalation recipient is configured, e.g. an `agent@team.host` address, with no atm-dev override) that recipient receives the same message as ordinary queued mail, else recorded as not run; `atm list --task-events` shows `reminded` rows with `blocked` and no Herdr prompt text in the pane; after the prompt is answered the next reminder is `emitted` |
