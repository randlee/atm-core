...
This sprint proves **queue**, not steer. An idle session may process the event through its ordinary runner pipeline. A busy matching Telegram session queues the internal event and drains it once after the active turn. It must not interrupt the turn or call steer. Queue is the current MVP seam, not a final product preference; an explicit ATM steer mode requires a later design, security review, and separate acceptance plan.
