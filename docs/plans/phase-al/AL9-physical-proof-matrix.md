...
| M5 direct cross-host write | `DirectPeerTcpConfig`, `direct_peer_tcp_client`, and `direct_peer_listener_uses_the_canonical_router_and_normalizes_provenance` | **Documented rerun required** | In a fresh M5 proof worktree, configure `ATM_HTTP_DIRECT_PEER_BIND=<private-m5-ip>:<port>` and `ATM_HTTP_DIRECT_PEER_SOURCE_HOST=<sender-host>` on the receiver, then `ATM_HTTP_DIRECT_PEER_PORT=<port>` on the sender. Curl a normal `WriteRequest` JSON to `POST /v1/atm/messages`; record source SHA/diff, HTTP response, persisted message, and exactly-one received-hook evidence. This is a temporary plaintext, unauthenticated evidence adapter only—not production activation. |
