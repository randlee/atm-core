...
| Cross-host/M5 | `operator=team-lead`; `status=documented-rerun-required` | Optional `DirectPeerTcpConfig` explicit plain-TCP listener plus `direct_peer_tcp_client` -> shared `HttpRuntimeClient` -> canonical `POST /v1/atm/messages` router | A fresh, create-run-discard M5 proof worktree configures receiver `ATM_HTTP_DIRECT_PEER_BIND=<private-m5-ip>:<port>` and `ATM_HTTP_DIRECT_PEER_SOURCE_HOST=<sender-host>`, sender `ATM_HTTP_DIRECT_PEER_PORT=<port>`, then records a curl `WriteRequest` proof | The explicit direct-peer listener is owned only by that proof runtime; it publishes no endpoint record and does not coexist with another direct-peer listener | No retirement/activation claim: temporary plaintext, unauthenticated proof adapter only | Omit `DirectPeerTcpConfig` or stop only the proof-owned runtime; retain the existing active runtime unchanged |
