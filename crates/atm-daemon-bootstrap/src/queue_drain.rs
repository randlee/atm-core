...
> {
    ...
    run_blocking("claim pending queue message", move || {
        ...
        let mut claim_guard =
            ClaimedPendingNudge::new(runtime_for_claim.clone(), member_for_claim.clone(), claim);
        let message_id = claim_guard.message_id();
...
async fn requeue_claim(
    runtime: &LocalServiceRuntime,
    member: &MemberKey,
    claim_guard: &ClaimedPendingNudge,
) -> Result<(), AtmError> {
...
struct ClaimedPendingNudge {
    runtime: LocalServiceRuntime,
    member: MemberKey,
    claim: Option<NudgeClaim>,
}

impl ClaimedPendingNudge {
...
impl Drop for ClaimedPendingNudge {
    fn drop(&mut self) {
