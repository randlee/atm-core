...
/// The search index has a document projection (`d`) as well as a canonical
/// message row (`m`). Mailbox reads and counts deliberately use the canonical
/// row for both: not every durability path is required to hydrate FTS before a
...
fn push_tag_filters(
    ...
    for tag in &filters.tags {
        clauses.push(
            format!("EXISTS (SELECT 1 FROM json_each(COALESCE({}.tags_json, '[]')) AS tag WHERE CAST(tag.value AS TEXT) = ?)", aliases.message),
        );
        parameters.push(SqlValue::Text(tag.clone()));
    }
    for tag in &filters.effective_tags {
        clauses.push(
            format!("EXISTS (SELECT 1 FROM json_each(COALESCE({}.effective_tags_json, '[]')) AS effective_tag WHERE CAST(effective_tag.value AS TEXT) = ?)", aliases.message),
        );
