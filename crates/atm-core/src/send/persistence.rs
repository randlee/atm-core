...
pub(crate) fn persist_message_with_ack_update(
    ...
    let (duplicate, already_closed) = mirror_message_to_store(
        runtime,
        home_dir,
        &recipient.team,
        &recipient.agent,
    ...
    Ok(persistence.with_already_closed(already_closed))
}
...
pub(crate) async fn persist_message_with_async_admission(
    ...
    let (duplicate, already_closed) = mirror_message_to_store_async(
        runtime,
        &recipient.team,
        &recipient.agent,
        &prepared,
...
> {
    let Some(message_id) = envelope.message_id else {
        return Ok((DuplicateWriteDisposition::NotDuplicate, None));
    };
    ...
    Ok((DuplicateWriteDisposition::NotDuplicate, None))
}
