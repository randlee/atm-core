...
pub fn prepare_peer_writes_atomically(
    ...
    prepare_peer_writes_atomically_with_runtime(requests, observability, runtime)
}
...
>(
    ...
    for request in requests {
        ...
        if envelope.parent_message_id.is_some() && envelope.thread_mode.is_some() {
            let inbox_messages = persistence::load_store_backed_mailbox_projection(
                runtime,
                &request.home_dir,
                &context.recipient.team,
        ...
        let persistence = match runtime.load_message_record(
            &request.home_dir,
            &context.recipient.team,
            &context.recipient.agent,
            &record.message_key,
        )? {
            Some(existing) => match persistence::classify_existing_message(
                existing,
                &envelope,
                message_id,
                &context.recipient.team,
                ...
                DuplicateWriteDisposition::AlreadyDeliveredRemote => {
                    DeliveryPersistenceResult::already_persisted(envelope)
                }
                DuplicateWriteDisposition::SameStorePeerReceipt => {
                    DeliveryPersistenceResult::same_store_peer_receipt(envelope)
                }
                DuplicateWriteDisposition::NotDuplicate => {
                    unreachable!("existing record is duplicate")
                }
            },
            None => {
                records.push(record);
    ...
    // array. Validation and duplicate conflict checks above complete before it
