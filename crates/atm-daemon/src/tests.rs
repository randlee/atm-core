...
fn dispatcher_send_after_add_member_roster_state_serializes_cleanly() {
    ...
    add_member_via_retained_admin(&db_path, &atm_home, TEST_TEAM, ROLE_TEAM_LEAD, &workspace_dir);
...
fn local_ipc_runtime_round_trips_send_after_add_member_roster_state() {
    ...
    let dispatcher: Arc<dyn RequestDispatcher + Send + Sync> = Arc::new(
        DaemonRequestDispatcher::new_for_test(
            atm_home.clone(),
            RuntimeStatusCache::new(),
            db_path.clone(),
        ),
