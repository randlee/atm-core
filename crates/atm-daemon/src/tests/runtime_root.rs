...
pub(crate) fn add_member_via_retained_admin(
...
pub(super) fn configure_trusted_peer(
    db_path: &std::path::Path,
    canonical_host: &str,
    aliases: &[&str],
) {
...
pub(super) fn configure_peer_http_source(db_path: &std::path::Path) {
    let assembly = open_sqlite_boundary(db_path).expect("sqlite boundary");
    assembly
        .peer_config_store()
    ...
    let server = thread::spawn(move || {
        for _ in 0..request_count {
            ...
            let ApiRequest::Write(write) = request else {
                panic!("direct peer sender must issue the ordinary shared write request");
            };
            let message_id = write
                .origin_message_id
                .expect("direct peer write preserves its immutable message ID");
            write_http_response(
                &mut stream,
