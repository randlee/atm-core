...
def _database_facts(path: Path, label: str) -> tuple[int, int]:
    ...
    try:
        # This function verifies files that are already hash/manifest bound
        # snapshot candidates or sidecar-free staging copies.  Immutable mode
        # prevents SQLite from trying to open a neighbouring WAL/SHM file when
        # macOS still has a harmless historical sidecar lock.
...
def restore_verified_snapshot(snapshot_id: str) -> VerifiedSnapshot:
    ...
    try:
        with closing(sqlite3.connect(
            f"file:{snapshot.database.resolve()}?mode=ro&immutable=1", uri=True,
        )) as reader:
