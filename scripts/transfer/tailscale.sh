...
# ATM_TEAM, plus an opt-in ATM_TRANSFER_SSH_CONFIG -- unset for ordinary
# installs), closed stdin, bounded deadline. Success is exactly one line on
# stdout: the landed directory's absolute path.
set -eu
...
# instead of the real `~/.ssh/config`; see sftp.sh's matching comment.
