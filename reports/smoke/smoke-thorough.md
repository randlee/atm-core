...
| `AG6-DOCTOR-VISIBILITY-001` | recorded doctor artifact shows the cross_host projection carrying durable interface state, bind visibility, and allowlist rows | `PASS` | artifact pair `reports/smoke/2026-07-16-ag6-doctor-visibility.{md,json}` captures the healthy doctor projection, while targeted tests also cover the empty-allowlist warning path and degraded/stale bind projection |
